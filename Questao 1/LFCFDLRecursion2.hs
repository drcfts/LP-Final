...
avaliar (LetR id expN corpo)    env = 
  avaliar corpo (avaliarCiclico id expN env)
  -- let
    -- e = EClosure expN env 
...
avaliarCiclico id exp env =
 let
   valueHolder = VInt 10          -- Valor "lixo" para inicializar o escopo da lista
   newEnv = (id, valueHolder):env
   valExpNomeada = avaliar exp newEnv  -- Esse sera o valor valido na lista que devera ser usado
 in
...
setBox id valor ((i,v): xs) 
 | id == i = ((i,valor): [])       -- Atualiza o valor da variavel pela recursao 
...
pesquisar v [] = error $ "Variavel " ++ show v ++ " nao declarada."
pesquisar v ((i,e):xs)
